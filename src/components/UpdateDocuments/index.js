...
const Documentation = ({ onPress }) => {
  ...
  const updateMemberdata = async () => {
    ...
    axios.request(config)
        .then((response) => {
            if (response.data.code == '200') {
                const docDetails=response?.data?.data?.docDetail?.uploadedDocuments
                const gstUrl=docDetails?.gst?.location[0]
                const panUrl=docDetails?.pan?.location[0]
                const adharUrl=docDetails?.aadhar?.location[0]
                const iecUrl=docDetails?.iecCertificate?.location[0]
                const selfieUrl=docDetails?.selfie?.location
                const bisUrl=docDetails?.bisCertificate?.location[0]

                const parts1 = gstUrl.split('/');
                const fileName1 = parts1[parts1.length - 1];
                const type1 = gstUrl.split('/');
                const fileType1=parts1[type1.length - 1];

                setGst(gstUrl)  
                setGstName(fileName1) 
                setGstType(`image/${fileType1}`)  
               

                const parts2 = panUrl.split('/');
                const fileName2 = parts2[parts2.length - 1];
                const type2 = panUrl.split('/');
                const fileType2=parts2[type2.length - 1];

                setPan(panUrl)
                setPanName(fileName2)
                setPanType(`image/${fileType2}`)

                const parts3 = adharUrl.split('/');
                const fileName3 = parts3[parts3.length - 1];
                const type3 = adharUrl.split('/');
                const fileType3=parts3[type3.length - 1];

                setAadhar(adharUrl)
                setAadharName(fileName3)
                setAadharType(`image/${fileType3}`)

                const parts5 = selfieUrl.split('/');
                const fileName5 = parts5[parts5.length - 1];
                const type5 = selfieUrl.split('/');
                const fileType5 =parts5[type5.length - 1];
          
                setPhoto(selfieUrl)
                setPhotoName(fileName5)
                setPhotoType(`image/${fileType5}`)

                const parts4 = iecUrl.split('/');
                const fileName4 = parts4[parts4.length - 1];
                const type4 = iecUrl.split('/');
                const fileType4=parts4[type4.length - 1];

                setIec(iecUrl)
                setIecName(fileName4)
                setIecType(`image/${fileType4}`)

                

                const parts6 = bisUrl.split('/');
                const fileName6 = parts6[parts6.length - 1];
                const type6 = bisUrl.split('/');
                const fileType6=parts6[type6.length - 1];

                setBis(bisUrl)
                setBisName(fileName6)
                setBisType(`image/${fileType6}`)
  ...
  const becomeAmember = async () => {
    ...
    if (personalAddress == '' || personalAddress == null) {
      ...
      let config = {
        method: 'post',
        url: `${Constants.MainUrl}member/update/details`,
        headers: {
          'Authorization': `${user_token}`,
